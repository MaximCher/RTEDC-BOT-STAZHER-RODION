...
async def record_payment_event(
    session: AsyncSession, payment_id: int, event_type: str, payload: Dict[str, Any]
) -> None:
    session.add(
        PaymentEvent(
    ...
    *,
    ...
    if signature.upper() != expected_sig.upper():
        await record_payment_event(
            session, payment.id, "callback_invalid_signature", normalized
        )
    ...
    await record_payment_event(session, payment.id, "callback_received", normalized)
