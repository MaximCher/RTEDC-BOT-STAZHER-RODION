...
class Payment(Base):
    ...
    provider: Mapped[str] = mapped_column(String(32), default="uniteller", index=True)
    ...
    callback_fields: Mapped[Optional[str]] = mapped_column(String(256), nullable=True)
    callback_format: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)

    response_code: Mapped[Optional[str]] = mapped_column(String(8), nullable=True)
    ...
    consultation_request_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("consultation_requests.id"), nullable=True, index=True
    )
